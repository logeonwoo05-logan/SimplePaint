...
- 구현한 내용 (위 그림 참조)
  - 그림판 캔버스에 그려진 그림을 이미지 파일로 저장하는 기능 구현함

  - 저장 버튼 클릭 시 SaveFileDialog가 실행되도록 구현함

  - PNG, JPG, BMP 형식으로 저장할 수 있도록 파일 필터 설정함

  - 사용자가 선택한 확장자에 맞게 이미지 저장 형식이 결정되도록 구현함

  - PictureBox에 표시된 Bitmap 이미지가 파일로 저장되도록 구현함

  - 저장 완료 후 파일이 정상적으로 생성되는지 테스트함
...
- 코드의 실행 스크린샷과 구현 내용 설명
  ...
  - 외부 이미지 파일을 불러오는 기능 구현함

  - 열기 버튼 클릭 시 OpenFileDialog가 실행되도록 구현함

  - PNG, JPG, BMP 이미지 파일을 선택할 수 있도록 파일 필터 설정함

  - 선택한 이미지 파일을 Bitmap으로 불러와 캔버스로 사용하도록 구현함

  - 불러온 이미지가 PictureBox에 표시되도록 구현함

  - 외부 이미지 위에 선택한 도형, 색상, 선 굵기로 그림을 그릴 수 있도록 구현함

  - 이미지 크기에 맞게 PictureBox 크기가 조정되도록 구현함

  - 불러온 이미지 위에 그린 결과를 저장할 수 있도록 구현함
